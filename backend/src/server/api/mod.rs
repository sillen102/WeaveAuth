...
/// swallowing.
pub(crate) async fn upgrade_bcrypt_to_argon2(users: &mut impl UserStorage, user_id: Uuid, password: String) {
    match crypto::hash_password(password).await {
        Ok(new_hash) => {
            if users.set_password(user_id, PasswordHash::Argon2(new_hash)).await != SetPasswordOutcome::Ok {
