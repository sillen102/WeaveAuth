...
impl SocketHost {
    pub(crate) fn new(
        ...
        Self { allowed, idle: Mutex::new(HashMap::new()), max_idle_per_endpoint, idle_timeout, io_timeout }
    }

    fn checkout(&self, host: &str, port: u16, tls: bool) -> Result<(Box<dyn Stream>, bool), String> {
        if !self.allowed.iter().any(|(h, p)| h == host && *p == port) {
            return Err(format!("endpoint {host}:{port} is not in the plugin socket allowlist"));
        }
        match self.take_idle(&(host.to_string(), port, tls)) {
            Some(stream) => Ok((stream, false)),
            None => Ok((self.dial(host, port, tls)?, true)),
        }
    }
    ...
    fn take_idle(&self, endpoint: &Endpoint) -> Option<Box<dyn Stream>> {
        let mut idle = self.idle.lock().ok()?;
        let slots = idle.get_mut(endpoint)?;
        while let Some(pooled) = slots.pop() {
            if pooled.returned_at.elapsed() < self.idle_timeout {
    ...
    fn dial(&self, host: &str, port: u16, tls: bool) -> Result<Box<dyn Stream>, String> {
        ...
        let stream = TcpStream::connect_timeout(&address, self.io_timeout)
            .map_err(|error| format!("cannot connect to {host}:{port}: {error}"))?;

        // The wasm timeout is wasmtime epoch interruption: it interrupts wasm
        // execution and cannot interrupt a host call blocked on a socket, so
        // without these a plugin waiting on a dead peer outlives its
        // configured timeout and holds a blocking-pool thread.
        stream.set_read_timeout(Some(self.io_timeout)).map_err(|error| error.to_string())?;
        stream.set_write_timeout(Some(self.io_timeout)).map_err(|error| error.to_string())?;

        if !tls {
            return Ok(Box::new(stream));
        }
        let connector = native_tls::TlsConnector::new().map_err(|error| format!("cannot build a TLS client: {error}"))?;
    ...
    fn checkin(&self, endpoint: Endpoint, stream: Box<dyn Stream>) {
        let Ok(mut idle) = self.idle.lock() else {
            return;
        };
        let slots = idle.entry(endpoint).or_default();
        if slots.len() < self.max_idle_per_endpoint {
            slots.push(Pooled { stream, returned_at: Instant::now() });
        }
    }
}
...
/// One plugin call's handle table. Handles are allocated per scope, so a
/// call can only name connections it opened itself, and anything still open
/// when the scope ends is closed rather than pooled -- the plugin never
/// declared it clean.
...
impl CallScope {
    /// Installs a scope for the current plugin call. The returned guard must
    /// outlive the call.
    pub(crate) fn enter(host: &Arc<SocketHost>) -> ScopeGuard {
    ...
    fn open(&self, host: &str, port: u16, tls: bool) -> Result<(u64, bool), String> {
        let (stream, fresh) = self.host.checkout(host, port, tls)?;
        let handle = self.next_handle.get();
        self.next_handle.set(handle + 1);
        self.open
            .borrow_mut()
            .insert(handle, Open { stream, endpoint: (host.to_string(), port, tls) });
        Ok((handle, fresh))
    }

    fn write(&self, handle: u64, data: &[u8]) -> Result<usize, String> {
        self.with_stream(handle, |stream| {
            stream.write_all(data).and_then(|()| stream.flush()).map_err(|error| error.to_string())?;
            Ok(data.len())
        })
    }

    fn read(&self, handle: u64, max: usize) -> Result<(Vec<u8>, bool), String> {
        self.with_stream(handle, |stream| {
    ...
    fn release(&self, handle: u64, reuse: bool) -> Result<(), String> {
        let open = self.open.borrow_mut().remove(&handle).ok_or_else(|| unknown_handle(handle))?;
        if reuse {
            self.host.checkin(open.endpoint, open.stream);
        }
        Ok(())
    }

    fn with_stream<T>(&self, handle: u64, f: impl FnOnce(&mut Box<dyn Stream>) -> Result<T, String>) -> Result<T, String> {
        let mut open = self.open.borrow_mut();
        let entry = open.get_mut(&handle).ok_or_else(|| unknown_handle(handle))?;
        f(&mut entry.stream)
    }
}
...
fn dispatch(request: &str, op: impl FnOnce(&CallScope, &str) -> Result<Value, String>) -> String {
    let scope = CURRENT_SCOPE.with(|current| current.borrow().clone());
    let result = match scope {
        Some(scope) => op(&scope, request),
        None => Err("the socket capability is not available in this call".to_string()),
    };
    ...
    match result {
        ...
        Err(message) => json!({"status": "error", "message": message}).to_string(),
    }
}

fn parse<'a, T: Deserialize<'a>>(request: &'a str, op: &str) -> Result<T, String> {
    serde_json::from_str(request).map_err(|error| format!("invalid {op} request: {error}"))
}
...
host_fn!(sock_write(_user_data: (); request: String) -> String {
    Ok(dispatch(&request, |scope, request| {
        ...
        let data = base64::engine::general_purpose::STANDARD
            .decode(&request.data)
            .map_err(|error| format!("sock_write data is not valid base64: {error}"))?;
...
mod tests {
    ...
    fn host(allowed: Vec<(String, u16)>, idle_timeout: Duration) -> Arc<SocketHost> {
        Arc::new(SocketHost::new(allowed, 8, idle_timeout, IO_TIMEOUT))
    }
    ...
    fn in_scope<T>(host: &Arc<SocketHost>, f: impl FnOnce(&CallScope) -> T) -> T {
        let _guard = CallScope::enter(host);
    ...
    fn rejects_an_unknown_handle() {
        ...
        assert!(write.expect_err("write fails").contains("7"), "the error must name the handle it refused");
        assert!(read.expect_err("read fails").contains("7"));
        assert!(release.expect_err("release fails").contains("7"));
    }
