...
WORKDIR /app
COPY . .
RUN cargo build --release -p weaveauth -p weaveauth-frontend -p weaveauth-login

# Stage 2: Runtime
FROM debian:bookworm-slim AS runtime
RUN apt-get update && apt-get install -y ca-certificates && rm -rf /var/lib/apt/lists/*
COPY --from=builder /app/target/release/weaveauth /usr/local/bin/weaveauth
COPY --from=builder /app/target/release/weaveauth-frontend /usr/local/bin/weaveauth-frontend
